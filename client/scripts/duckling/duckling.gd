...
# 주의: 이 레이는 절대 높이(예전 y=10)에서 쏘면 안 된다 — 섬 트라이메쉬에는 나무가 통째로
# 포함돼 있어서, 나무 캐노피 아래를 지나기만 해도 캐노피 꼭대기가 "지면"으로 측정돼
# 새끼오리가 나무 위로 떠올라 앉는 버그가 있었다. 시작점을 "현재 발 높이 + 밟을 수 있는
# 최대 턱 높이(STEP_HEIGHT)"로 제한하면 머리 위 캐노피는 탐색에 아예 안 걸리고, 실제로
# 걸어 올라갈 수 있는 표면만 지면으로 인정된다(섬 오르기는 계단 오르기가 발 높이를
# 먼저 올려주므로 그대로 동작한다).
...
# 섬 해안선 같은 낮은 턱을 자연스럽게 올라가기 위한 계단 오르기. CharacterBody3D는
# floor_max_angle을 넘는 수직 턱에 그냥 막히기 때문에, 오리는 넘어가는 턱을 새끼오리만
# 못 넘어 섬 아래에 남겨지는 문제가 있었다. 알고리즘과 튜닝값(STEP_HEIGHT,
# STEP_CHECK_DISTANCE)은 player.gd의 것을 그대로 참조한다 — 리더(오리)가 넘을 수 있는
# 턱과 정확히 같은 턱만 넘어야 하고, 값 튜닝도 player.gd 한 곳에서만 이루어지게.
const PlayerScript := preload("res://scripts/player/player.gd")
const STEP_PROBE_HALF_WIDTH := 0.288 # player.gd와 같은 비율(콜리전 폭(0.6) * 0.48)
# 계단을 밟기 전 "땅을 딛고 있는가" 판정 허용 오차. 플레이어는 is_on_floor()를 쓰지만,
# 새끼오리는 중력 없이 목표 높이로 lerp만 하므로 바닥을 누르는 속도가 없어 is_on_floor()가
# 사실상 항상 false다. 대신 이미 측정해둔 지면 높이(_ground_height_cached)와의 차이로
# 같은 조건을 재현한다 — 이 게이트가 없으면 한 번 올라간 뒤 공중에 뜬 채로 연속으로
# 계단을 밟아 STEP_HEIGHT보다 훨씬 높은 경사면도 사다리 타듯 올라가 버린다.
const STEP_GROUNDED_TOLERANCE := 0.05
...
# player.gd _apply_step_up()/_probe_step_up()을 그대로 옮긴 것(상수도 그쪽 것을 참조).
# 차이는 둘뿐이다: (1) 땅 딛음 판정 — is_on_floor() 대신 지면 높이 비교(STEP_GROUNDED_TOLERANCE
# 주석 참고), (2) 레이 마스크 — exclude=[self] 대신 지형 전용 레이어만 검사(플레이어나
# 다른 새끼오리 몸통을 턱으로 오인해 올라타지 않도록).
func _apply_step_up() -> void:
	if position.y > _ground_height_cached + STEP_GROUNDED_TOLERANCE:
		return
...
func _probe_step_up(origin: Vector3, direction: Vector3) -> float:
	...
	var step_height: float = PlayerScript.STEP_HEIGHT
	var check_distance: float = PlayerScript.STEP_CHECK_DISTANCE

	# 발목 높이에서 진행 방향으로 지형이 막고 있는지 확인
	var low_from := origin + Vector3(0, 0.1, 0)
	var low_query := PhysicsRayQueryParameters3D.create(
		low_from, low_from + direction * check_distance, GROUND_QUERY_MASK)
	...
	var high_query := PhysicsRayQueryParameters3D.create(
		high_from, high_from + direction * check_distance, GROUND_QUERY_MASK)
	if not space_state.intersect_ray(high_query).is_empty():
		return 0.0

	# 턱 너머의 실제 바닥 높이를 아래로 레이캐스트해서 얼마나 들어올려야 하는지 확인
	var probe_top := high_from + direction * check_distance
...
func _ground_height_at(x: float, z: float) -> float:
	...
	var query := PhysicsRayQueryParameters3D.create(
		Vector3(x, position.y + PlayerScript.STEP_HEIGHT, z),
