...
경찰(`tagger`)이 대시를 시작한다. 클라이언트가 대시 시작/도착 지점을 계산해 함께 보내면, 서버는 그 사각형 경로(`DASH_CATCH_HALF_WIDTH`만큼의 폭)에 오리가 겹치는지 판정해 자동으로 수감시킨다.
...
- 서버는 이 메시지를 받은 즉시(다음 판정 틱이 아니라 이 메시지 처리 시점에) 대시 경로와 오리 위치를 비교해 겹치면 수감 처리하고 `player_jailed` 이벤트를 보낸다. 지연 없이 "부딪힌 순간 = 수감" 이 되도록 구현하는 것이 중요하다(연출상 그렇게 맞춰져 있음).
...
8. **대시 판정**: `player:dash` 메시지를 받으면 즉시(다음 틱을 기다리지 않고) 시작/도착 좌표로 만든 선분을 기준으로, 수감되지 않은 모든 오리와의 최단 거리가 `DASH_CATCH_HALF_WIDTH` 이하인지 확인해 겹치면 그 오리를 즉시 수감(`jail_player`) + `player_jailed` 이벤트. 이때 들고 있던 새끼오리는 잡힌 자리 근처에 흩뿌려지며 다시 `spawned`로 돌아간다.
